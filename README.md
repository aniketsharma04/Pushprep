...
pushprep defaults to Google's floating `gemini-flash-lite-latest` alias — the cheapest model per commit, since Flash-Lite spends no reasoning tokens on a task that doesn't need them, so a free-tier key lasts far longer. It keeps a **fallback chain** of known-good models behind it: if a model has been retired for your API key, is out of quota, or rejects the request, pushprep transparently tries the next one instead of silently degrading. Run `pushprep doctor` any time to confirm your key and a model are actually reachable.
